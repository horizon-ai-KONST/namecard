...
export function createApp({ withStatic = false } = {}) {
  ...
  // ---- Config: brand + enabled modes (consumed by frontend on load) ----
  // Set PRODUCT_FLAVOR=konst in env to switch the deployment to Konst branding.
  // MODES_ENABLED overrides the default list, comma-separated.
  app.get('/api/config', (req, res) => {
    const FLAVORS = {
      horizon: {
        brand_name: 'HORIZON AI',
        brand_tagline: '名片夾',
        watermark: 'Powered by Konst',
        default_modes: ['tradeshow', 'daily', 'visit'],
        has_display_screen: true,
      },
      konst: {
        brand_name: '»KONST',
        brand_tagline: '名片管理',
        brand_subtagline: 'Where Efficiency Leads, Where AI Defines the Future.',
        watermark: 'KONST · 康斯特科技',
        default_modes: ['daily', 'visit'],
        has_display_screen: false,
      },
    };
    const flavor = (process.env.PRODUCT_FLAVOR || 'horizon').toLowerCase();
    const preset = FLAVORS[flavor] || FLAVORS.horizon;

    const modesOverride = process.env.MODES_ENABLED
      ? process.env.MODES_ENABLED.split(',').map((s) => s.trim()).filter(Boolean)
      : null;

    res.json({
      flavor,
      brand: {
        name: process.env.BRAND_NAME || preset.brand_name,
        tagline: process.env.BRAND_TAGLINE || preset.brand_tagline,
        subtagline: process.env.BRAND_SUBTAGLINE || preset.brand_subtagline || '',
        watermark: process.env.BRAND_WATERMARK || preset.watermark,
      },
      modes: modesOverride || preset.default_modes,
      has_display_screen: preset.has_display_screen,
      bitable_url: process.env.BITABLE_VIEW_URL || '',
    });
  });
