...
const FIELD_ALIASES = {
  ...
  mode: ['模式', 'Mode'],
  source_context: ['來源情境', '展會名稱', 'Source', 'Event'],
  visit_note: ['拜訪備註', '會議備註', 'Visit Note', 'Meeting Note'],
};
...
const DESIRED_SCHEMA = [
  ...
  { key: 'source_context', name: '來源情境', type: FIELD_TYPES.TEXT },
  { key: 'visit_note', name: '拜訪備註', type: FIELD_TYPES.TEXT },
];
...
export async function ensureSchema() {
  ...
  if (created.length > 0) invalidateSchemaCache();
  return { created };
}

// Schema lookups are expensive (a full listFields call). Cache the resolved
// mapping for 10 minutes since fields rarely change at runtime.
let schemaCache = { data: null, expiresAt: 0 };
const SCHEMA_TTL_MS = 10 * 60 * 1000;

export async function resolveSchema({ force = false } = {}) {
  if (!force && schemaCache.data && Date.now() < schemaCache.expiresAt) {
  ...
  const data = { fields, resolved };
  schemaCache = { data, expiresAt: Date.now() + SCHEMA_TTL_MS };
  return data;
}

// Bust the cache after running ensureSchema (new fields were just created).
function invalidateSchemaCache() {
  schemaCache = { data: null, expiresAt: 0 };
}
...
// `context` is { mode, event_name, visit_note } from the Console.
// Returns { record_id, fields } describing the created row.
export async function saveCard(card, imageBuffer, context = {}) {
  ...
  // mode + source_context + visit_note (from Console)
  const modeMap = { tradeshow: '展場', daily: '日常', visit: '拜訪' };
  if (context.mode && modeMap[context.mode]) {
    put('mode', modeMap[context.mode]);
  }
  // source_context: event name for tradeshow, otherwise descriptive label
  const sourceContext = context.event_name
    || (context.mode === 'daily' ? '日常' : context.mode === 'visit' ? '客戶拜訪' : '');
  put('source_context', sourceContext);
  put('visit_note', context.visit_note);
...
// Fetch a single record by id (for resend-greeting flow).
